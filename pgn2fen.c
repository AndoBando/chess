...
void fprintfwboard(FILE* stream, boardstate* p_b) {
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "hpo:e:s:k0lcw")) != -1) {
		switch (opt) {
		...
		case '0': break;
		case 'l': lenient = true; break;
		case 'c': fatal = false; break;
		case 'w': fwmode = true; break;
		case 'h':
			fprintf(stderr, "PGN to FEN converter\n"
			        "Usage: %s (input files...) [-hpoesk0lc] [file...]\n\n", argv[0]);
			fprintf(stderr,
			        ...
			        "-k, keep tags, leeps the tags given in the pgn file. By default\n"
			        "    they are thrown out.\n"
			        "-l, lenient, ignores notated takes 'x', checks '+' and mate '#'.\n"
			        "    by default 'x' is required if a piece is taken, and if  is \n"
			        "    written, we verify that check actually happens.\n"
			        "-c, continue, tries to continue when reaching an error, still \n"
			        "    still prints a message to stderr. By default errors halt \n"
			        "    the program\n"
			        "-w, fixed width mode, outputs a form similar to fen, but in a\n"
	...
	if (optind == 1) {
		fprintf(stderr, "Usage: %s (input files...) [-cehkops0w] [file...]\n"
		        "-h for help\n"
